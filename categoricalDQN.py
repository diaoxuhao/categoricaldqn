...
class Networks(object):
    ...
    def value_distribution_network(input_shape, num_atoms, action_size, learning_rate):
        state_input = keras.layers.Input(shape=(input_shape)) 
        cnn_feature = keras.layers.Conv2D(32, (8, 8), strides=(4,4), activation='relu')(state_input)
        cnn_feature = keras.layers.Conv2D(64, (4, 4), strides=(2,2), activation='relu')(cnn_feature)
        cnn_feature = keras.layers.Conv2D(64, (3, 3), activation='relu')(cnn_feature)
        cnn_feature = keras.layers.Flatten()(cnn_feature)
        cnn_feature = keras.layers.Dense(512, activation='relu')(cnn_feature)
        distribution_list = []
        for i in range(action_size):
            distribution_list.append(keras.layers.Dense(num_atoms, activation='softmax')(cnn_feature))
        model = keras.models.Model(inputs=state_input, outputs=distribution_list)
...
#
#        # Epsilon Greedy
        action_idx  = agent.get_action(x_t)
        a_t[action_idx] = 1
#
        a_t = a_t.astype(int)
...
#        r_t = game.get_last_reward()  #each frame we get reward of 0.1, so 4 frames will be 0.4
        ...
        if (is_terminated):
            ...
            x_t1 = env.reset() 
        x_t1 = np.reshape(x_t1, (1, img_rows, img_cols,img_channels))
        ...
        if t % 1000 == 0:
            ...
            agent.model.save_weights("result.dxh", overwrite=True)
