...
class Main:
    ...
    def replay_memory(self, s_t, action, r_t, s_t1, is_terminated, t):
        ...
        if self.epsilon > self.final_epsilon and t > self.observe+self.explore:
            self.epsilon = self.epsilon - (self.epsilon - self.final_epsilon) * min(1,(t-self.observe-self.explore) / (self.explore_frac * self.Max_t))
...
if __name__ == '__main__':
    ...
    for game in games:
        ...
        if len(sys.argv)>1 and sys.argv[1] == 'load':
            print('Load model')
            agent.model.load_weights('./Breakout.dxh')
        is_terminated = 0
...
#            print(r_t, R)
            ...
            if (is_terminated):
                ...
                result = '(episode=' + str(GAME) +' Reward='\
                         + str(R) + ' t=' + str(t) + ' explore=' +\
                           str(agent.epsilon)  + '),\n'
                file.write(result)
                file.flush()
                x_t1 = env.reset()
